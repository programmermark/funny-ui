...
describe('test ripple component props', () => {
  test('test ripple show & hide', async () => {
    ...
    await delay(60)
    expect(wrapper.find('.fac-ripple').exists()).toBeTruthy()

    await trigger(wrapper, 'touchend')
    await delay(1000)
    expect(wrapper.find('.fac-ripple').exists()).toBeFalsy()
    wrapper.unmount()
  })
  ...
  test('test ripple update & color', async () => {
    ...
    await delay(60)
    expect(wrapper.find('.fac-ripple').exists()).toBeTruthy()
    expect(wrapper.find('.fac-ripple').element.style.backgroundColor).toBe('green')
